...
function validRecord(record) {
    return record
        ...
        && ['pending', 'review', 'acknowledged', 'ignored'].includes(record.status);
}
...
class PendingWinStore {
    ...
    prune() {
        ...
        this.records = this.records.filter(record => (
            ...
            || Number(record.acknowledgedAt || record.ignoredAt || 0) >= cutoff
        ));
        return this.records;
    }
    ...
    add({
        ...
        if (existing) {
            let changed = false;
            for (const key of ['senderName', 'accountName']) {
        ...
        const record = {
            ...
            ...normalized,
            status: status === 'review' ? 'review' : 'pending',
            detectedAt: this.now(),
            lastNotifiedAt: 0,
            notifyCount: 0,
        };
    ...
    dueReviews() {
        return this.reviews().filter(record => Number(record.notifyCount || 0) === 0);
    }
    ...
    acknowledgeByMessages(talkerId, messages, myUid) {
        ...
        for (const record of this.records) {
            if (!['pending', 'review'].includes(record.status)
                || record.talkerId !== toId(talkerId)) continue;
    ...
    ignoreWhere(predicate, reason = 'filtered-non-winner') {
        this.load();
        const now = this.now();
        const ignored = [];
        for (const record of this.records) {
            if (!['pending', 'review'].includes(record.status) || !predicate(record)) continue;
            record.status = 'ignored';
            record.ignoredAt = now;
            record.ignoredReason = String(reason || 'filtered-non-winner');
            ignored.push(record);
        }
        if (ignored.length) this.save();
        return ignored;
    }
    ...
    reclassify(classify) {
        this.load();
        const now = this.now();
        const changes = { pending: [], review: [], ignored: [] };
        for (const record of this.records) {
            if (!['pending', 'review'].includes(record.status)) continue;
            const classification = classify(record);
            const nextStatus = classification === 'winner'
                ? 'pending'
                : classification === 'review' ? 'review' : 'ignored';
            if (record.status === nextStatus) continue;
            record.status = nextStatus;
            record.reclassifiedAt = now;
            if (nextStatus === 'ignored') {
                record.ignoredAt = now;
                record.ignoredReason = 'classified-non-winner-message';
            } else {
                delete record.ignoredAt;
                delete record.ignoredReason;
            }
            changes[nextStatus].push(record);
        }
        if (changes.pending.length || changes.review.length || changes.ignored.length) this.save();
        return changes;
    }
    ...
    markNotified(recordIds) {
        ...
        let changed = false;
        for (const record of this.records) {
            if (!['pending', 'review'].includes(record.status) || !ids.has(record.id)) continue;
