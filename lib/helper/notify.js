...
async function sendNotify(text, desp, params = {}) {
    const instanceName = String(process.env.LOTTERY_INSTANCE_NAME || '').trim();
    const instancePrefix = instanceName ? `[${instanceName}] ` : '';
    if (instancePrefix && !String(text).startsWith(instancePrefix)) {
        text = instancePrefix + text;
    }
    if (process.env.NOTE) {
