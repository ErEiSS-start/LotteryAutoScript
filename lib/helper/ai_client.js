...
function getModelState(provider) {
    const key = [provider?.url || '', provider?.body?.model || provider?.name || 'unknown'].join('|');
...
function getAiProviderAvailability(providers = [], now = Date.now) {
    ...
    providers.forEach((provider, index) => {
        ...
        configured += 1;
        const providerOpenUntil = getProviderState(provider, index).openUntil;
        const modelOpenUntil = getModelState(provider).openUntil;
        const blockedUntil = Math.max(providerOpenUntil, modelOpenUntil);
        if (blockedUntil <= now()) available += 1;
        else nextAvailableAt = Math.min(nextAvailableAt, blockedUntil);
    });
    return {
        configured,
        available,
        allUnavailable: configured === 0 || available === 0,
        nextAvailableAt: Number.isFinite(nextAvailableAt) ? nextAvailableAt : 0,
    };
}
...
async function requestAiWithFailover({
    ...
    for (const { provider, originalIndex: index } of ordered.entries) {
        ...
        const modelState = getModelState(provider);
        if (modelState.openUntil > now()) {
            if (!modelState.openWarningLogged) {
                log.warn(`${purpose}主备`, `${provider.body?.model || name}模型繁忙冷却中，尝试其他模型`);
                modelState.openWarningLogged = true;
            }
            continue;
        }
        modelState.openWarningLogged = false;
        ...
        for (let attempt = 0; attempt <= retryCount; attempt++) {
            ...
            modelBusy = /(?:\b1305\b|模型当前访问量过大|模型.*(?:繁忙|拥堵))/i.test(reason);
            rateLimited = !modelBusy
                && /(?:HTTP状态码:\s*429|\b1302\b|rate.?limit|速率限制|请求过于频繁)/i.test(reason);
            log.warn(
                `${purpose}主备`,
                `${name}第${attempt + 1}次失败${attempt < retryCount && !rateLimited && !modelBusy ? '，准备重试' : ''}: ${reason}`
            );
            if (rateLimited || modelBusy) break;
        }

        if (modelBusy) {
            modelState.openUntil = now() + modelBusyCooldown;
            modelState.openWarningLogged = true;
            log.warn(
                `${purpose}主备`,
                `${provider.body?.model || name}模型公共资源繁忙，模型级冷却${modelBusyCooldown / 60000}分钟`
            );
            continue;
        }
