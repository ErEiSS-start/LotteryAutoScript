...
async function judgeLotteryInfo(lotteryInfo, {
    ...
    if (!response) {
        failedThisRun.add(failureKey);
        return {
            result: null,
            failed: true,
            attempted: true,
            stopBatch: getAiProviderAvailability(providers, now).allUnavailable,
        };
    }
    ...
    for (let index = 0; index < candidates.length; index++) {
        ...
        if (judged?.stopBatch) {
            ...
            if (remainingFallback) {
                log.warn('AI预判', `所有智谱账号当前均不可用，剩余${remainingFallback}条本次任务使用关键词筛选`);
            }
            break;
        }
