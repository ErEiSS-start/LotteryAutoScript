...
class Monitor extends Searcher {
     ...
     * - 评论 达到当前发送上限后停评并继续参与（内部状态，不向上返回错误）
     * - 关注 未知错误 2001
     * - 关注 您已被对方拉入黑名单 2002
     * - 关注 黑名单用户无法关注 2003
     * - 关注 账号异常 2004
    ...
    async go(option) {
        ...
        /* 评论 */
        if (rid && chat_type && !global_var.get('commentDisabled')) {
            if (is_copy_chat) {
            ...
            if (status === 13) {
                const disabledAccounts = global_var.get('commentDisabledAccounts') instanceof Set
                    ? global_var.get('commentDisabledAccounts')
                    : new Set();
                disabledAccounts.add(String(global_var.get('accountNum')));
                global_var.set('commentDisabledAccounts', disabledAccounts);
                global_var.set('commentDisabled', true);
                log.warn(
                    '评论降级',
                    `帐号${global_var.get('accountNum')}已达到评论发送上限；本次进程后续候选跳过评论，继续执行其他参与动作`
                );
                status = 0;
            } else if (status) {
