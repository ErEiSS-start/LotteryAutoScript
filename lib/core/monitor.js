...
const {
    log, hasEnv, isEnvEnabled, matchesLotteryKeywords, shuffle, getRandomOne, delay,
...
const { judgeLotteryInfo } = require('../helper/ai_judge');
        ...
        await try_for_each(protoLotteryInfo, async function (lottery_info) {
            ...
            if (!hasOfficialLottery && model[1] === '1') {
                if (isEnvEnabled('ENABLE_AI_JUDGE')) {
                    const judged = await judgeLotteryInfo(lottery_info);
                    if (judged?.result) {
                        const aiResult = judged.result;
                        aiJudged = true;
                        has_key_words = aiResult.is_lottery;
                        needAt = aiResult.need_at;
                        needTopic = aiResult.need_topic;
                        drawtime = aiResult.draw_time;
                        const drawTimePassed = drawtime > 0 && drawtime < now_ts;
                        isLottery = aiResult.is_lottery && !aiResult.is_ended && !drawTimePassed;
                        if (aiResult.is_lottery && (aiResult.is_ended || drawTimePassed)) {
                            log.info('AI过滤', `已结束(https://t.bilibili.com/${dyid})`);
                        }
                        log.debug(
                            'AI判断抽奖',
                            `${dyid} provider=${judged.provider} lottery=${aiResult.is_lottery} ended=${aiResult.is_ended} drawtime=${drawtime} reason=${aiResult.reason}`
                        );
                    } else {
                        log.debug('AI判断抽奖', `${dyid}主备均失败，使用关键词规则`);
                    }
                }
                if (!has_key_words && description && !aiJudged) {
