...
const {
    log, hasEnv, isEnvEnabled, matchesLotteryKeywords, shuffle, getRandomOne, getAiContent, delay,
...
const { CommentPackStore } = require('../helper/comment_pack_store');
const { requestAiWithFailover, normalizeAiProviders } = require('../helper/ai_client');
const { discoveryState } = require('../helper/discovery_state');
const { RoundRobinQueue } = require('../helper/round_robin_queue');

const successfulCommentsByDyid = new Map();
const followRestrictedTargetsByAccount = new Map();
let legacyAiJudgeConfigWarned = false;

/**
 * 兼容旧版AI判断：只直连一个接口，不读取缓存、轮换密钥或故障切换。
 * 新版providers配置只取第一个可用项，避免要求既有my_config.js迁移回旧字段。
 * @param {object} aiJudgeParm
 * @returns {{url: string, body: object, prompt: string, apiKeyEnv: string}|null}
 */
function getLegacyAiJudgeRequest(aiJudgeParm = {}) {
    const legacyUrl = String(aiJudgeParm?.url || '').trim();
    const provider = legacyUrl
        ? aiJudgeParm
        : Array.isArray(aiJudgeParm?.providers)
            ? aiJudgeParm.providers.find(item => String(item?.url || '').trim())
            : null;
    if (!provider) return null;

    const url = String(provider.url || '').trim();
    const prompt = String(aiJudgeParm.prompt || provider.prompt || '').trim();
    if (!url || !prompt) return null;
    return {
        url,
        body: provider.body && typeof provider.body === 'object' ? provider.body : {},
        prompt,
        apiKeyEnv: String(
            provider.api_key_env || aiJudgeParm.api_key_env || 'AI_API_KEY'
        ).trim() || 'AI_API_KEY',
    };
}

function rememberFollowRestriction(accountUid, targetUid) {
    const account = String(accountUid || '').trim();
    const target = String(targetUid || '').trim();
    if (!account || !target) return [];
    const targets = followRestrictedTargetsByAccount.get(account) || new Set();
    targets.add(target);
    followRestrictedTargetsByAccount.set(account, targets);
    return [...targets];
}

function getFollowRestrictedTargets(accountUid) {
    return [...(followRestrictedTargetsByAccount.get(String(accountUid || '').trim()) || [])];
}

function clearFollowRestrictions(accountUid) {
    if (accountUid === undefined) followRestrictedTargetsByAccount.clear();
    else followRestrictedTargetsByAccount.delete(String(accountUid || '').trim());
}

function followRestrictionThreshold(value) {
    return Math.max(2, Number(value) || 2);
}

function shouldEscalateFollowRestriction(accountUid, threshold) {
    return getFollowRestrictedTargets(accountUid).length
        >= followRestrictionThreshold(threshold);
}
...
const DEFAULT_UNIQUE_COMMENT_FALLBACKS = [
    ...
    '来试一回', '等个好消息', '希望轮到我', '来碰碰手气',
    '想试试运气', '安静等结果', '来占个位', '期待好运',
    '看看这次', '来参与一回', '希望有机会', '等一份惊喜',
    '试试手气吧', '期待好结果', '来凑个数', '希望被抽中',
    '静候结果', '来试一下', '等一个机会', '期待开奖',
    '希望这次中', '来蹲一下', '想碰碰运气', '等个惊喜',
    '这次试试看', '希望好运来', '参与试试', '来等结果',
    '期待有我', '希望中一次', '来看看运气', '蹲一下结果',
    '试试这一回', '等着开奖', '期待被选中', '希望有份',
    '来等好消息', '碰一碰运气', '想参与看看', '希望抽到我',
    '安静蹲结果', '期待这一回', '来试试机会', '希望有惊喜',
];

function deferFailedRelay(queue, lottery) {
    if (!queue || !lottery) return false;
    // 转发发生在评论之后。候选稍后重试时不应再次发送已经成功的评论。
    delete lottery.rid;
    delete lottery.chat;
    lottery.chat_type = 0;
    return queue.defer(lottery.dyid);
}
...
const COMMENT_JSON_SCHEMA = {
    type: 'json_schema',
    json_schema: {
        name: 'bilibili_lottery_comment',
        strict: true,
        schema: {
            type: 'object',
            ...
            properties: {
                comment: { type: 'string' },
                has_explicit_requirement: { type: 'boolean' },
            },
            required: ['comment', 'has_explicit_requirement'],
        },
    },
};
const COMMENT_PACK_JSON_SCHEMA = {
    type: 'json_schema',
    json_schema: {
        name: 'bilibili_lottery_comment_pack',
        strict: true,
        schema: {
            type: 'object',
            additionalProperties: false,
            ...
            required: ['comments', 'has_explicit_requirement'],
        },
    },
};
let
...
function getCommentProviders(parm = {}, jsonSchema = COMMENT_JSON_SCHEMA) {
    return normalizeAiProviders(parm)
        .map(provider => ({
            ...provider,
...
function validateAiCommentResponseJson(response) {
...
function parseAiCommentPackResponse(response) {
    const raw = String(response || '')
        .trim()
        .replace(/^```(?:json)?\s*/i, '')
        .replace(/\s*```$/i, '')
    ...
    try {
        ...
        if (parsed && Array.isArray(parsed.comments)) {
            return {
                comments: parsed.comments.map(comment => String(comment || '').trim()),
                hasExplicitRequirement: parsed.has_explicit_requirement === true,
            };
        }
    } catch (_) {
...
function getCommentPackStore() {
    if (!commentPackStore) {
        commentPackStore = new CommentPackStore({
            retentionDays: config.ai_comment_history_days,
        });
    }
    return commentPackStore;
}
...
function getCommentPackContext(env = process.env) {
    let accounts = [];
    try {
        const parsed = JSON.parse(String(env.LOTTERY_COMMENT_ACCOUNT_NUMBERS || '[]'));
        if (Array.isArray(parsed)) {
            accounts = parsed
                .map(account => String(account))
                .filter((account, index, values) => account && values.indexOf(account) === index);
        }
    } catch (_) {
        accounts = [];
    }
    const currentAccount = String(env.NUMBER || '1');
    if (!accounts.length) accounts = [currentAccount];
    let slot = Number(env.LOTTERY_COMMENT_SLOT);
    if (!Number.isSafeInteger(slot) || slot < 0 || slot >= accounts.length) {
        slot = Math.max(0, accounts.indexOf(currentAccount));
    }
    return { accounts, slot };
}

function getCommentPackSignature(dyid, source, accounts, parm = config.ai_comments_parm) {
    const signatureData = {
        version: 1,
        dyid: String(dyid),
        source: String(source || ''),
        accounts: accounts.map(String),
        prompt: String(parm?.prompt || ''),
        providers: Array.isArray(parm?.providers)
            ? parm.providers.map(provider => ({
                name: provider?.name,
                url: provider?.url,
            ...
            : [],
        body: parm?.body || {},
        maxLength: Number(config.ai_comment_short_max_length) || 15,
        similarityThreshold: Number(config.ai_comment_similarity_threshold) || 0.57,
    };
    return crypto.createHash('sha256').update(JSON.stringify(signatureData)).digest('hex');
}

function buildAiCommentPackPrompt(basePrompt, { accountCount, previousComments }) {
    const previous = previousComments.length
        ? `\n这些评论近期已经使用过，尽量不要复用：${previousComments.slice(-20).join('；')}。`
        : '';
    return `${String(basePrompt || '').trim()}
你是普通B站用户，正在参与动态活动。请只输出JSON：
{"comments":["评论1","评论2"],"has_explicit_requirement":false}
comments必须恰好包含${accountCount}条，按不同用户分别生成。
规则：
1. 如果原文明示必须评论指定口令、选项或答案，严格照做并将has_explicit_requirement设为true；此时允许多条相同。
2. 仅要求“评论”但未指定内容时，has_explicit_requirement必须为false。
3. 普通评论每条只写2至15个汉字，${accountCount}条之间不得复用措辞、句式或信息顺序。
4. 不总结或改写活动文案，不复述群号、长数字、链接、进群、购买或抢购引导。
5. 不使用表情、话题标签、@、营销口吻，也不编造使用体验。
6. 只返回JSON，不要解释。${previous}`.trim();
}

function selectCommentPackFallback(dyid, slot, usedComments, recentNormalized, threshold) {
    const start = stableCommentOffset(dyid, slot + 1) % DEFAULT_UNIQUE_COMMENT_FALLBACKS.length;
    const ordered = Array.from(
        { length: DEFAULT_UNIQUE_COMMENT_FALLBACKS.length },
        (_, index) => DEFAULT_UNIQUE_COMMENT_FALLBACKS[(start + index) % DEFAULT_UNIQUE_COMMENT_FALLBACKS.length]
    );
    return ordered.find(comment =>
        !recentNormalized.has(normalizeAiComment(comment))
        && getMaxAiCommentSimilarity(comment, usedComments) < threshold
    ) || ordered.find(comment =>
        getMaxAiCommentSimilarity(comment, usedComments) < threshold
    ) || ordered[0];
}

function completeCommentPack({
    dyid,
    accountCount,
    parsed,
    usedComments,
    ...
    const explicit = parsed?.hasExplicitRequirement === true;
    const sourceComments = Array.isArray(parsed?.comments) ? parsed.comments : [];
    const comments = [];
    let aiCount = 0;

    for (let slot = 0; slot < accountCount; slot++) {
        const candidate = String(sourceComments[slot] || '').trim();
        const quality = validateAiComment(candidate, {
            hasExplicitRequirement: explicit,
            maxLength,
        });
        const normalized = normalizeAiComment(candidate);
        const acceptable = quality.valid && (
            explicit
            || (
                getMaxAiCommentSimilarity(candidate, usedComments) < similarityThreshold
                && !recentNormalized.has(normalized)
            )
        );
        if (acceptable) {
            comments.push(candidate);
            usedComments.set(normalized, candidate);
            aiCount += 1;
            continue;
        }

        if (explicit) {
            const explicitFallback = comments[0]
                || sourceComments.find(comment =>
                    validateAiComment(comment, { hasExplicitRequirement: true, maxLength }).valid
                );
            if (explicitFallback) {
                comments.push(String(explicitFallback).trim());
                aiCount += 1;
                continue;
            }
        }

        const fallback = selectCommentPackFallback(
            dyid,
            slot,
            usedComments,
            recentNormalized,
        ...
        usedComments.set(normalizeAiComment(fallback), fallback);
    }
    ...
    return {
        ...
        hasExplicitRequirement: explicit,
        aiCount,
        localCount: Math.max(0, accountCount - aiCount),
    };
}

async function getPackedAiComment(dyid, source) {
    const { accounts, slot } = getCommentPackContext();
    const signature = getCommentPackSignature(dyid, source, accounts);
    const store = getCommentPackStore();
    const cached = store.get(dyid, signature);
    if (cached?.comments?.[slot]) {
        log.debug('AI评论包', `dyid(${dyid})帐号槽位${slot + 1}命中缓存`);
        return cached.comments[slot];
    }

    const usedComments = new Map(getSuccessfulComments(dyid));
    ...
    const parm = config.ai_comments_parm || {};
    const providers = getCommentProviders(parm, COMMENT_PACK_JSON_SCHEMA);
    const aiResponse = await requestAiWithFailover({
        providers,
        prompt: buildAiCommentPackPrompt(parm.prompt, {
            accountCount: accounts.length,
            previousComments: recentComments.map(entry => entry.comment),
        }),
        content: source,
        validate: parseAiCommentPackResponse,
        purpose: 'AI评论包',
        timeout: Math.max(5000, Number(config.ai_request_timeout) || 30000),
        retryCount: Math.max(0, Number(config.ai_comment_provider_retry_count) || 0),
    ...
    const pack = completeCommentPack({
        dyid,
        accountCount: accounts.length,
    ...
    store.remember({
        ...
        hasExplicitRequirement: pack.hasExplicitRequirement,
        provider: aiResponse?.provider || 'local',
        aiCount: pack.aiCount,
        localCount: pack.localCount,
    });
    const summary = `dyid(${dyid})生成${accounts.length}条：AI ${pack.aiCount}，本地补齐${pack.localCount}，provider=${aiResponse?.provider || 'local'}`;
    if (pack.localCount) log.warn('AI评论包', summary);
    else log.info('AI评论包', summary);
    return pack.comments[slot];
}
...
async function getUniqueAiComment(dyid, source) {
    ...
    for (let attempt = 0; attempt <= retryCount; attempt++) {
        const aiResponse = await requestAiWithFailover({
            providers: getCommentProviders({
                ...config.ai_comments_parm,
                url,
                body,
            }),
            prompt: buildAiCommentPrompt(prompt, {
                accountNumber,
                dyid,
                attempt,
            ...
            content: source,
            validate: validateAiCommentResponseJson,
            purpose: 'AI评论',
            timeout: Math.max(5000, Number(config.ai_request_timeout) || 30000),
...
class Monitor extends Searcher {
    /**
     * @constructor
     * @param {[string, number | string, string?]} lottery_param
     */
    constructor(lottery_param) {
        super();
        this.lottery_param = lottery_param;
        this.sourceId = lottery_param[2] || '';
    ...
    async init() {
        ...
        if (process.env.LOTTERY_DISCOVERY_ONLY) {
            ...
            if (!global_var.get('discoveryIncomplete') && this.sourceId) {
                if (discoveryState.markCompleted(this.sourceId)) {
                    const progress = discoveryState.getProgress();
                    log.info('轮转采集', `来源进度 ${progress.completed}/${progress.total}`);
                }
            }
        ...
        switch (status) {
            ...
            case 4005:
                {
                    const accountUid = global_var.get('myUID');
                    const restrictedTargets = getFollowRestrictedTargets(accountUid);
                    const detail = status === 2004
                        ? `本次任务已有${restrictedTargets.length}个不同UP返回关注限制：${restrictedTargets.join('、')}`
                        : '点赞接口返回账号异常';
                    log.warn(`账号异常${status}`, `UID(${accountUid}) ${detail}`);
                }
                if (!config.is_exception) {
                    const restrictedTargets = getFollowRestrictedTargets(global_var.get('myUID'));
                    await sendNotify(
                        status === 2004
                            ? '[动态抽奖]多UP关注受限2004通知'
                            : `[动态抽奖]账号异常${status}通知`,
     ...
     */
    async startLottery() {
        const { create_dy, create_dy_mode, wait, filter_wait } = config
            , isBatchMode = Boolean(process.env.LOTTERY_SHARED_ONLY)
            , batchSize = Math.max(1, Number(config.lottery_batch_size) || 8)
        ...
        if (isBatchMode) {
            queue = new RoundRobinQueue({ account: Number(process.env.NUMBER) });
            const loaded = await queue.loadOrCreate({
                snapshotFilename: process.env.LOTTERY_SHARED_SNAPSHOT_FILE || 'lottery_info_1.json',
                config,
                createCandidates: () => this.filterLotteryInfo(),
            });
            allLottery = [...queue.pending];
            const summary = queue.summary();
            log.info(
                '持久队列',
                `帐号${summary.account}${loaded.created ? '新建' : '续接'}队列：初始${summary.initial}，已成功${summary.successful}，已跳过${summary.skipped}，剩余${summary.remaining}`
            );
        } else {
            allLottery = await this.filterLotteryInfo();
        }
        ...
        if (len) {
            ...
            const candidates = isBatchMode ? allLottery : shuffle(allLottery);
            for (const [index, lottery] of candidates.entries()) {
                total_nums += 1;
                let
                    is_shutdown = false,
                    relayDeferred = false;
                if (
                    ...
                    if (queue) queue.defer(lottery.dyid);
                    continue;
                }

                if (isEnvEnabled('ENABLE_AI_JUDGE') && lottery.drawtime !== -1 && lottery.drawtime < Date.now() / 1000) {
                    log.info('AI过滤', '已过开奖时间');
                    await d_storage.updateDyid(lottery.dyid);
                    if (queue) queue.complete(lottery.dyid, 'skipped');
                ...
                const interactionSucceeded = status === 0;
                ...
                switch (status) {
                    ...
                    case 2004:
                        {
                            const accountUid = global_var.get('myUID');
                            const restrictedTargets = getFollowRestrictedTargets(accountUid);
                            const threshold = followRestrictionThreshold(
                                config.follow_restriction_distinct_up_threshold
                            );
                            if (shouldEscalateFollowRestriction(accountUid, threshold)) {
                                is_exception = 2004;
                            } else {
                                log.warn(
                                    '目标UP关注受限',
                                    `本次仅${restrictedTargets.length}个不同UP返回22015，未达到账号级告警阈值${threshold}，不发送推送`
                                );
                                status = 0;
                            }
                        }
                    ...
                    case 2001:
                        ...
                        break;
                    case 5001:
                        if (isBatchMode && deferFailedRelay(queue, lottery)) {
                            relayDeferred = true;
                            status = 0;
                        } else {
                            is_shutdown = true;
                ...
                if (relayDeferred) {
                    log.warn(
                        '轮转批次',
                        `转发结果暂时无法确认：dyid(${lottery.dyid})已移到队尾，后续轮次重试且不重复评论`
                    );
                    continue;
                }
                ...
                await d_storage.updateDyid(lottery.dyid);
                if (queue) queue.complete(lottery.dyid, interactionSucceeded ? 'successful' : 'skipped');
                completedThisRound += 1;

                await delay(wait * (Math.random() + 0.5));

                if (isBatchMode && batchResult.successful >= batchSize) {
                    batchResult.needsAnotherRound = queue.remaining > 0;
                    log.info(
                        '轮转批次',
            ...
            if (queue) {
                batchResult.remaining = queue.remaining;
                if (!pauseCurrentRound && !batchResult.errorStatus && batchResult.successful < batchSize) {
                    batchResult.needsAnotherRound = queue.remaining > 0 && completedThisRound > 0;
                    if (queue.remaining > 0 && completedThisRound === 0) {
                        log.warn('持久队列', `本轮没有可完成候选，保留剩余${queue.remaining}条到下次任务`);
                    }
                }
            }
    ...
    async filterLotteryInfo() {
        ...
        /** 所有抽奖信息 */
        let alllotteryinfo = [];
        const quietSharedFilter = Boolean(process.env.LOTTERY_SHARED_ONLY);
        const filterSummary = new Map();
        const configuredSummaryLimit = Number(config.lottery_filter_log_detail_limit);
        const summaryLimit = Number.isFinite(configuredSummaryLimit)
            ? Math.max(0, configuredSummaryLimit)
            : 5;
        const recordFilter = (reason, dyid, detail = '') => {
            if (!quietSharedFilter) return false;
            const entry = filterSummary.get(reason) || { count: 0, samples: [] };
            entry.count += 1;
            if (entry.samples.length < summaryLimit) {
                const sample = detail
                    ? `${dyid}:${String(detail).replace(/\s+/g, ' ').slice(0, 100)}`
                    : String(dyid);
                entry.samples.push(sample);
            }
            filterSummary.set(reason, entry);
            return true;
        };
        const
            {
        ...
        if (save_lottery_info_to_file
            && (protoLotteryInfo.length || process.env.LOTTERY_DISCOVERY_ONLY)
            && (lottery_param[0] !== 'APIs' || process.env.LOTTERY_DISCOVERY_ONLY)) {
            log.info('保存抽奖信息', '保存开始');
            await appendLotteryInfoFile(this.sourceId || lottery_param[1].toString(), protoLotteryInfo);
        }
        ...
        await try_for_each(protoLotteryInfo, async function (lottery_info) {
            ...
            let
                ...
                has_key_words = matchesLotteryKeywords(description, key_words),
            ...
            if (!hasOfficialLottery && model[1] === '1') {
                if (isEnvEnabled('ENABLE_AI_JUDGE')) {
                    const aiJudgeRequest = getLegacyAiJudgeRequest(config.ai_judge_parm);
                    if (!aiJudgeRequest) {
                        if (!legacyAiJudgeConfigWarned) {
                            legacyAiJudgeConfigWarned = true;
                            log.warn('AI判断抽奖', '未配置可用的AI判断接口，使用关键词规则');
                        }
                    } else {
                        const msg = await getAiContent(
                            aiJudgeRequest.url,
                            aiJudgeRequest.body,
                            aiJudgeRequest.prompt,
                            lottery_info.des,
                            { apiKeyEnv: aiJudgeRequest.apiKeyEnv }
                        );
                        try {
                            const msgJson = JSON.parse(msg);
                            if (typeof msgJson.has_key_words === 'boolean') {
                                has_key_words = msgJson.has_key_words;
                                isLottery = has_key_words;
                            } else {
                                log.warn('AI判断抽奖', 'no has_key_words');
                            }
                            if (typeof msgJson.needAt === 'boolean') {
                                needAt = msgJson.needAt;
                            } else {
                                log.warn('AI判断抽奖', 'no needAt');
                            }
                            if (typeof msgJson.needTopic === 'string') {
                                needTopic = msgJson.needTopic;
                            } else {
                                log.warn('AI判断抽奖', 'no needTopic');
                            }
                            if (typeof msgJson.drawtime === 'number') {
                                drawtime = msgJson.drawtime;
                            } else {
                                log.warn('AI判断抽奖', 'no drawtime');
                            }
                            log.info('AI判断抽奖', msgJson.more);
                        } catch (_) {
                            log.error('AI判断抽奖', '未返回有效JSON，使用关键词规则');
                        }
                    }
                }
                if (!has_key_words && description) {
                    if (!recordFilter('无关键词', dyid, description)) {
            ...
            if (isLottery) {
                ...
                if (!recordFilter('非抽奖', dyid)) {
                    log.info('筛选动态', `非抽奖动态(https://t.bilibili.com/${dyid})`);
                }
            }
        });

        if (quietSharedFilter && filterSummary.size) {
            const summary = [...filterSummary.entries()].map(([reason, entry]) => {
                const samples = entry.samples.length ? `，示例[${entry.samples.join('；')}]` : '';
                return `${reason}${entry.count}${samples}`;
            }).join(' | ');
            log.info('筛选汇总', summary);
        }
    ...
    async go(option) {
        ...
        if (uid.length) {
            await try_for_each(uid, async (u) => {
                ...
                if (status) {
                    if (status === 4) {
                        const targets = rememberFollowRestriction(global_var.get('myUID'), u);
                        log.warn(
                            '目标UP关注受限',
                            `dyid: ${dyid}, uid: ${u}, 本次任务不同受限UP数: ${targets.length}`
                        );
                    }
        ...
        if (rid && chat_type) {
            if (useAiComment && !is_copy_chat) {
                ...
                chat = config.ai_comment_pack_enabled !== false
                    ? await getPackedAiComment(dyid, aiCommentSource)
                    : await getUniqueAiComment(dyid, aiCommentSource);
                isAiChat = true;
