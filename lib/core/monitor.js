...
function getSuccessfulComments(dyid) {
    if (!successfulCommentsByDyid.has(dyid)) {
        successfulCommentsByDyid.set(dyid, new Map());
    }
...
function rememberSuccessfulComment(dyid, comment) {
    ...
    if (normalized) getSuccessfulComments(dyid).set(normalized, String(comment).trim());
}
...
async function getUniqueAiComment(dyid, source, uname) {
    ...
    const accountHint = `\n这是帐号${accountNumber}的独立评论，请勿提及帐号编号，并使用自然且有区别的措辞。`;

    let comment = await getAiContent(url, body, `${prompt}${accountHint}`, source);
    let normalized = normalizeAiComment(comment);
    if (normalized && !usedComments.has(normalized)) return comment.trim();

    const previous = [...usedComments.values()];
    log.warn('AI评论去重', `dyid(${dyid})首次结果为空或重复，携带已有评论重试一次`);
    comment = await getAiContent(
        url,
        body,
        `${prompt}${accountHint}\n请换一种表达，不得与以下评论相同：${previous.join('；')}`,
        source
    );
    normalized = normalizeAiComment(comment);
    if (normalized && !usedComments.has(normalized)) return comment.trim();

    const configuredComments = Array.isArray(config.chat)
        ? config.chat.map(value => String(value).replace(/\$\{uname\}/g, uname || ''))
        : [];
    const fallback = selectUniqueCommentFallback(
        [...configuredComments, ...DEFAULT_UNIQUE_COMMENT_FALLBACKS],
        usedComments,
        accountNumber - 1
    );
    log.warn('AI评论去重', `dyid(${dyid})改用未重复的本地评论`);
    return fallback || DEFAULT_UNIQUE_COMMENT_FALLBACKS[(accountNumber - 1) % DEFAULT_UNIQUE_COMMENT_FALLBACKS.length];
}
            ...
            if (isLottery) {
                ...
                if (isSendChat) {
                    ...
                    if (hasEnv('ENABLE_AI_COMMENTS')) {
                        ...
                        onelotteryinfo.aiCommentUname = uname;
                    } else {
    ...
    async go(option) {
        ...
        let
            status = 0,
            {
                ...
                useAiComment, aiCommentSource, aiCommentUname,
        ...
        if (rid && chat_type) {
            if (useAiComment && !is_copy_chat) {
                ...
                chat = await getUniqueAiComment(dyid, aiCommentSource, aiCommentUname);
                isAiChat = true;
            ...
            if (status === 8 ||
                status === 9) {
                postedChat = selectUniqueCommentFallback(
