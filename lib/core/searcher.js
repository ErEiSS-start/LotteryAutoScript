...
function shouldStopDiscoveryRisk(scope) {
    ...
    if (process.env.LOTTERY_DISCOVERY_ONLY) {
        markDiscoveryIncomplete(`${scope}因-352/412熔断停止`);
    }
...
class Searcher {
    ...
    static async checkAllDynamic(host_mid, pages, time = 0, offset = '0', options = {}) {
        ...
        for (let i = 0; i < pages; i++) {
            ...
            if (process.env.LOTTERY_DISCOVERY_ONLY) {
                ...
                if (shouldStopDiscoveryRisk(`${options.scope || `用户${host_mid}`}动态分页`)) {
                    return null;
                }
                mDRdata = modifyDynamicRes(oneDynamicInfo);
                if (mDRdata !== null) bili.recordDiscoverySuccess('空间动态分页');
            } else for (let attempt = 0; attempt <= retry352Cooldowns.length; attempt++) {
    ...
    async getLotteryInfoByUID(UID) {
        ...
        for (const cur of dynamicList) {
            if (shouldStopDiscoveryRisk(`用户${UID}的动态详情`)) break;
            ...
            if (!card && shouldStopDiscoveryRisk(`用户${UID}的动态详情`)) break;
            if (!card) {
    ...
    async getLotteryInfoByCollectionUID(UID) {
        ...
        for (const collection of candidates) {
            if (shouldStopDiscoveryRisk(`合集用户${UID}的动态详情`)) break;
            ...
            for (const dyid of dyids) {
                if (shouldStopDiscoveryRisk(`合集用户${UID}的动态详情`)) break collectionLoop;
                seenDyids.add(dyid);
                const card = await bili.getOneDynamicByDyid(dyid);
                if (!card && shouldStopDiscoveryRisk(`合集用户${UID}的动态详情`)) break collectionLoop;
                if (!card) {
    ...
    async getLotteryInfoByArticle(key_words) {
        ...
        for (const { id, pub_time } of cvs) {
            if (shouldStopDiscoveryRisk('专栏动态详情')) break;
            ...
            for (const dyid of dyid_set) {
                if (shouldStopDiscoveryRisk('专栏动态详情')) break articleLoop;
                log.info('获取动态', `查看专栏中所提及动态(${dyid}) (${length--})`);
                const card = await bili.getOneDynamicByDyid(dyid);
                if (!card && shouldStopDiscoveryRisk('专栏动态详情')) break articleLoop;
                                        ...
                                        const lottery_info = raw_lottery_info
                                            .reduce(async (pre, cur) => {
                                                ...
                                                if (!check_if_duplicated || check_if_duplicated >= 2) {
                                                    if (shouldStopDiscoveryRisk('API动态详情')) return results;

                                                    log.info('获取动态', `查看动态(${dyid})是否点赞 (${length--})`);
                                                    const card = await bili.getOneDynamicByDyid(dyid);
                                                    if (!card && shouldStopDiscoveryRisk('API动态详情')) return results;
    ...
    async getLotteryInfoByTxT(txt) {
        ...
        for (const dyid of dyids) {
            if (shouldStopDiscoveryRisk('Txt动态详情')) break;

            log.info('获取动态', `查看Txt中所提及动态(${dyid}) (${length--})`);
            const card = await bili.getOneDynamicByDyid(dyid);
            if (!card && shouldStopDiscoveryRisk('Txt动态详情')) break;
