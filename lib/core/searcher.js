...
const {
    ...
    collection_dynamic_keywords, search_wait, tag_scan_page
} = require('../data/config');
const d_storage = require('../helper/d_storage');
const global_var = require('../data/global_var');

const { log } = utils;
let nextDynamicDetailBreakerLogAt = 0;
...
function shouldStopDynamicDetailScan(scope) {
    if (!bili.isDynamicDetailCircuitOpen()) {
        nextDynamicDetailBreakerLogAt = 0;
        return false;
    }
    ...
    if (process.env.LOTTERY_DISCOVERY_ONLY) {
        ...
        const reason = `${scope}因412熔断停止`;
        if (!reasons.includes(reason)) reasons.push(reason);
        global_var.set('discoveryIncompleteReasons', reasons);
    }
    if (now >= nextDynamicDetailBreakerLogAt) {
        const remainingMinutes = Math.max(
            1,
            Math.ceil(bili.getDynamicDetailCircuitRemainingMs() / 60000)
        );
        log.warn(
            '动态详情熔断',
            `${scope}停止继续扫描，约${remainingMinutes}分钟后允许探测；其他任务继续运行`
        );
        nextDynamicDetailBreakerLogAt = now + 60 * 1000;
    }
    return true;
}
...
class Searcher {
    ...
    async getLotteryInfoByUID(UID) {
        ...
        for (const cur of dynamicList) {
            if (shouldStopDynamicDetailScan(`用户${UID}的动态详情`)) break;
            ...
            if (!card && shouldStopDynamicDetailScan(`用户${UID}的动态详情`)) break;
            if (card) {
    ...
    async getLotteryInfoByCollectionUID(UID) {
        ...
        const allDynamic = await Searcher.checkAllDynamic(UID, scanPages, search_wait);
        if (allDynamic === null) return null;
        ...
        log.info('获取合集动态', `用户${UID}找到疑似合集动态(${candidates.length})`);

        const seenDyids = new Set();
        const dyinfos = [];
        collectionLoop:
        for (const collection of candidates) {
            if (shouldStopDynamicDetailScan(`合集用户${UID}的动态详情`)) break;
            ...
            for (const shortId of extracted.shortIds) {
                ...
                if (dyid) shortDyids.push(dyid);
            }
            const dyids = [...new Set([...extracted.dyids, ...shortDyids])]
                .filter(dyid => dyid !== collectionDyid && !seenDyids.has(dyid));
            log.info('获取合集动态', `合集动态(${collectionDyid})提取链接动态ID(${dyids.length})`);

            for (const dyid of dyids) {
                if (shouldStopDynamicDetailScan(`合集用户${UID}的动态详情`)) break collectionLoop;
                seenDyids.add(dyid);
                const card = await bili.getOneDynamicByDyid(dyid);
                if (!card && shouldStopDynamicDetailScan(`合集用户${UID}的动态详情`)) break collectionLoop;
                if (!card) {
    ...
    async getLotteryInfoByTag(tag_name) {
        ...
        const tag_id = await bili.getTagIDByTagName(tag_name);
        if (Number(tag_id) < 0) {
            log.warn('获取动态', `话题#${tag_name}#旧接口不可用，本轮跳过该来源`);
            return [];
        }

        const hotdy = await bili.getHotDynamicInfoByTagID(tag_id);
        if (!hotdy) return [];
        const modDR = modifyDynamicRes(hotdy);
        ...
        for (let index = 0; index < tag_scan_page; index++) {
            ...
            const
                ...
                _modify = newdy ? modifyDynamicRes(newdy) : null;

            if (_modify === null) break;
    ...
    async getLotteryInfoByArticle(key_words) {
        ...
        for (const { id, pub_time } of cvs) {
            if (shouldStopDynamicDetailScan('专栏动态详情')) break;
            ...
            for (const dyid of dyid_set) {
                if (shouldStopDynamicDetailScan('专栏动态详情')) break articleLoop;
                log.info('获取动态', `查看专栏中所提及动态(${dyid}) (${length--})`);
                const card = await bili.getOneDynamicByDyid(dyid);
                if (!card && shouldStopDynamicDetailScan('专栏动态详情')) break articleLoop;
                                        ...
                                        const lottery_info = raw_lottery_info
                                            .reduce(async (pre, cur) => {
                                                ...
                                                if (!check_if_duplicated || check_if_duplicated >= 2) {
                                                    if (shouldStopDynamicDetailScan('API动态详情')) return results;

                                                    log.info('获取动态', `查看动态(${dyid})是否点赞 (${length--})`);
                                                    const card = await bili.getOneDynamicByDyid(dyid);
                                                    if (!card && shouldStopDynamicDetailScan('API动态详情')) return results;
    ...
    async getLotteryInfoByTxT(txt) {
        ...
        for (const dyid of dyids) {
            if (shouldStopDynamicDetailScan('Txt动态详情')) break;

            log.info('获取动态', `查看Txt中所提及动态(${dyid}) (${length--})`);
            const card = await bili.getOneDynamicByDyid(dyid);
            if (!card && shouldStopDynamicDetailScan('Txt动态详情')) break;
...
module.exports = { Searcher, parseDynamicCard, extractCollectionDynamicLinks };
