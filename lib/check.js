...
const WIN_MESSAGE_CLASS = Object.freeze({
    WINNER: 'winner',
    REVIEW: 'review',
    IGNORED: 'ignored',
});

function classifyWinnerMessage(content, noticeKeyWords) {
    const text = messageText(content).replace(/\s+/g, ' ').trim();
    if (!text) return WIN_MESSAGE_CLASS.IGNORED;
    ...
    if (hardNonWinnerSignals.some(pattern => pattern.test(text))) {
        return WIN_MESSAGE_CLASS.IGNORED;
    }
    ...
    if (strongWinnerSignals.some(pattern => pattern.test(text))) {
        return WIN_MESSAGE_CLASS.WINNER;
    }

    // 通用促销口令只能在没有明确个人中奖信号时屏蔽，避免误伤
    // “恭喜中奖，请扫码领取红包”这类真实领奖通知。
    const promotionalSignals = [
        /【?必得红包】?/,
        /(?:支付宝|微信)?.{0,8}(?:扫码|点击|打开).{0,12}(?:领|领取|抢).{0,8}(?:红包|优惠券|福利)/,
        /(?:扫码|点击|打开).{0,12}(?:领|领取|抢).{0,8}(?:红包|优惠券|福利)/,
        /(?:天天|每日|人人|新用户).{0,10}(?:可领|领取|领).{0,8}(?:红包|优惠券|福利)/,
    ];
    if (promotionalSignals.some(pattern => pattern.test(text))) {
        return WIN_MESSAGE_CLASS.IGNORED;
    }
    ...
    if (nonWinnerSignals.some(pattern => pattern.test(text))) {
        return WIN_MESSAGE_CLASS.IGNORED;
    }

    // 自定义关键词继续保留高召回率，但单独命中“码、获得、填写”等弱词时
    // 只进入一次性人工复核，不再反复作为确定中奖提醒。
...
function isWinnerMessage(content, noticeKeyWords) {
    return classifyWinnerMessage(content, noticeKeyWords) !== WIN_MESSAGE_CLASS.IGNORED;
}
...
function reviewMessageDescription(records) {
    let desp = '';
    for (const record of records) {
        desp += '## 疑似中奖私信（仅提醒一次）\n\n';
        desp += '- - - -\n\n';
        ...
        desp += `中奖账号: ${identity(record.accountName, record.accountUid)}\n\n`;
        desp += `私信内容:\n${record.content}\n\n`;
        desp += `[前往查看](${record.link})\n\n`;
        desp += '该消息只有弱中奖线索，脚本不会重复提醒；请在中奖管理页人工归档。\n\n';
        desp += '- - - -\n\n';
    }
    return desp;
}

function publicWinnerDescription({
    heading,
    timestamp,
    senderName,
    senderUid,
    ...
    let desp = `## ${heading}\n\n`;
    desp += '- - - -\n\n';
    desp += `发生时间: ${new Date(Number(timestamp) * 1000).toLocaleString()}\n\n`;
    desp += `发信人: ${identity(senderName, senderUid)}\n\n`;
    desp += `中奖账号: ${identity(accountName, accountUid)}\n\n`;
    desp += `通知类型: ${notificationType}\n\n`;
    desp += `原内容:\n${content}\n\n`;
    desp += `[直达链接](${link})\n\n`;
    desp += '- - - -\n\n';
    return desp;
...
async function isMe(num) {
    ...
    pendingStore.load();
    const reclassified = pendingStore.reclassify(
        record => classifyWinnerMessage(record.content, notice_key_words)
    );
    if (reclassified.ignored.length) {
        log.notice(
            '待领取中奖记录',
            `自动归档${reclassified.ignored.length}条预约确认、促销、调研或活动邀请误报`
        );
    }
    if (reclassified.review.length) {
        log.notice(
            '待确认中奖记录',
            `将${reclassified.review.length}条仅有弱中奖线索的记录改为一次性人工确认`
        );
    }
    if (reclassified.pending.length) {
        log.notice(
            '待领取中奖记录',
            `将${reclassified.pending.length}条已具备明确中奖信号的记录恢复为持续提醒`
        );
    }
    ...
    if (unread_at_num) {
        ...
        MyAtInfo
            ...
            .forEach(({ at_time, senderName, senderUid, business, source_content, url }) => {
                desp += publicWinnerDescription({
                    heading: '[at]检测结果',
                    timestamp: at_time,
                    senderName,
                    senderUid,
                    ...
                    accountUid: myUid,
                    notificationType: `在${business || '动态'}中@了中奖账号`,
    ...
    if (unread_reply_num) {
        ...
        replys
            ...
            .forEach(({ senderName, senderUid, uri, source, timestamp }) => {
                const classification = classifyWinnerMessage(source, notice_key_words);
                if (classification !== WIN_MESSAGE_CLASS.IGNORED) {
                    desp += publicWinnerDescription({
                        heading: classification === WIN_MESSAGE_CLASS.WINNER
                            ? '回复检测结果'
                            : '疑似中奖回复',
                        timestamp,
                        senderName,
                ...
                for (const Session of MySession.data) {
                    ...
                    if (unread_count) {
                        ...
                        for (const message of messages) {
                            ...
                            const classification = classifyWinnerMessage(content, notice_key_words);
                            if (classification === WIN_MESSAGE_CLASS.IGNORED) continue;
                            const result = pendingStore.add({
                                ...
                                status: classification === WIN_MESSAGE_CLASS.WINNER
                                    ? 'pending'
                                    : 'review',
                            });
                            if (result?.created) {
                                log.notice(
                                    classification === WIN_MESSAGE_CLASS.WINNER
                                        ? '待领取中奖记录'
                                        : '待确认中奖记录',
                                    classification === WIN_MESSAGE_CLASS.WINNER
                                        ? `新增帐号${num}与用户${talker_id}的中奖私信，保持未读`
                                        : `新增帐号${num}与用户${talker_id}的疑似中奖私信，仅提醒一次`
                                );
                            }
                        }
                    }
    ...
    for (const record of [...pendingStore.pending(), ...pendingStore.reviews()]) {
    ...
    const reviewRecords = pendingStore.dueReviews();
    let reviewDesp = '';
    if (reviewRecords.length) {
        const enrichedReviews = await enrichPrivateMessageRecords(
            reviewRecords,
            global_var.get('myUNAME') || ''
        );
        reviewDesp = reviewMessageDescription(enrichedReviews);
    }
    ...
    if (desp) {
        ...
        await sendNotify(
            dueRecords.length
                ? `帐号${num}有${dueRecords.length}条中奖信息待领取`
                : `帐号${num}可能中奖了`,
            desp
        );
    ...
    if (reviewDesp) {
        log.notice('疑似中奖待确认', reviewDesp);
        await sendNotify(
            `帐号${num}有${reviewRecords.length}条疑似中奖待确认`,
            reviewDesp
        );
        pendingStore.markNotified(reviewRecords.map(record => record.id));
    }

    if (!desp && !reviewDesp) {
        const pendingCount = pendingStore.pending().length;
        const reviewCount = pendingStore.reviews().length;
        log.notice(
            '中奖检测',
            pendingCount
                ? `有${pendingCount}条待领取记录，尚未到下次提醒时间`
                : reviewCount
                    ? `有${reviewCount}条疑似中奖待确认，均已完成一次提醒`
                    : '暂未中奖'
        );
    }
    return;
