...
const utils = {
    ...
    /** 双服务器部署时用于区分日志和推送来源。 */
    instanceName(env = process.env) {
        return String(env.LOTTERY_INSTANCE_NAME || '').trim();
    },
    accountName(number = process.env.NUMBER, env = process.env) {
        const instance = utils.instanceName(env);
        return `${instance ? `${instance} ` : ''}帐号${number}`;
    },
    /**
    ...
    * infiniteNumber() {
        ...
        debug(context, msg) {
            if (this._level >= 4) {
                ...
                let color_text_pair = [
                    ...
                    [this._colors[2], `[${utils.accountName()} ${context}]`],
                    [this._colors[3], `[\n${msg}\n]`],
                ];
     ...
     * @param {{apiKey?: string, apiKeyEnv?: string, timeout?: number}} [options]
     * @returns {Promise<string|null>}
     */
    getAiContent(url, body, prompt, content, options = {}) {
        const apiKeyEnv = String(options.apiKeyEnv || 'AI_API_KEY').trim() || 'AI_API_KEY';
        const apiKey = options.apiKey ?? process.env[apiKeyEnv];
        const timeout = Math.max(1, Number(options.timeout) || 120000);
        if (!String(url || '').trim() || !apiKey) return Promise.resolve(null);
        return new Promise((resolve) => {
            send({
                method: 'POST',
                url,
