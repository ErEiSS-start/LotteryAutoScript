...
const utils = {
    ...
    * infiniteNumber() {
    ...
    async appendLotteryInfoFile(from, lottery_info) {
        ...
        const filename = `lottery_info_${Number(process.env.NUMBER)}.next.json`;
        try {
        ...
        await utils.createFile(utils.lottery_info_dir, filename, JSON.stringify(all_lottery_info), 'w');
    },
    /**
     * 读取lottery_info
     * @param {string} filename
    ...
    async clearLotteryInfo() {
        await utils.createDir(utils.lottery_info_dir);
        await utils.createFile(utils.lottery_info_dir, `lottery_info_${Number(process.env.NUMBER)}.next.json`, '{}', 'w');
    },
    /**
     * 完整扫描结束后提交本轮抽奖信息
    ...
    async commitLotteryInfo(maxAgeDays = Infinity) {
        ...
        const nextItems = Object.values(nextObject)
            .flat()
            .filter(item => item && typeof item === 'object' && item.dyid);
        if (!nextItems.length) {
        ...
        let previousObject = {};
        try {
            previousObject = utils.strToJson(fs.readFileSync(finalPath).toString());
        } catch (_) {
            previousObject = {};
        }

        const previousItems = Object.values(previousObject)
                .flat()
                .filter(item => item && typeof item === 'object' && item.dyid),
            ageDays = Number(maxAgeDays),
        ...
        utils.log.info(
            '保存抽奖信息',
            `共享文件更新完成，本轮${nextItems.length}条，合并去重后${lotteryMap.size}条`
        );
        return true;
    },
    /**
