...
function send(detail) {
    ...
    if (!headers['user-agent']) options.headers['user-agent'] = process.env.ACCOUNT_UA || DEFAULT_UA;
    ...
    const req = request(options, res => {
        ...
        if (redirect && ~~(statusCode / 100) === 3) {
            const redirectUrl = resolveRedirectUrl(headers.location, thisURL);
            if (!redirectUrl) {
                failure(`[响应错误]无效重定向地址: ${headers.location || ''}`);
                res.resume();
                return;
            }
...
module.exports = { send, resolveRedirectUrl };
