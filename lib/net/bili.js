...
const DEFAULT_DISCOVERY_RISK_THRESHOLD = 3;
const DEFAULT_DISCOVERY_RISK_COOLDOWN = 2 * 60 * 1000;
const DEFAULT_DISCOVERY_RISK_RETRY_WAIT = 3 * 1000;
...
const DYNAMIC_DETAIL_RISK_FAILURE = Symbol('dynamic-detail-risk-failure');
...
class DynamicDetailCircuitBreaker {
    ...
    get threshold() {
        const threshold = Number(
            process.env.LOTTERY_DISCOVERY_ONLY
                ? config.discovery_risk_threshold
                : config.dynamic_detail_412_threshold
        );
        return Number.isFinite(threshold) && threshold > 0
            ? Math.floor(threshold)
            : (process.env.LOTTERY_DISCOVERY_ONLY
                ? DEFAULT_DISCOVERY_RISK_THRESHOLD
    ...
    get cooldown() {
        const cooldown = Number(
            process.env.LOTTERY_DISCOVERY_ONLY
                ? config.discovery_risk_cooldown
                : config.dynamic_detail_412_cooldown
        );
        return Number.isFinite(cooldown) && cooldown > 0
            ? cooldown
            : (process.env.LOTTERY_DISCOVERY_ONLY
                ? DEFAULT_DISCOVERY_RISK_COOLDOWN
                : DEFAULT_DYNAMIC_DETAIL_412_COOLDOWN);
    }

    get retryWait() {
        const retryWait = Number(config.discovery_risk_retry_wait);
        return Number.isFinite(retryWait) && retryWait > 0
            ? retryWait
            : DEFAULT_DISCOVERY_RISK_RETRY_WAIT;
    }
    ...
    recordSuccess() {
        const recovered = this.probing || this.consecutiveRisk > 0;
        this.reset();
        return { recovered };
    }
    ...
    recordFailure(responseText, now = Date.now()) {
        ...
        if (!code) {
            return { isRisk: false, opened: false, count: this.consecutiveRisk };
        }

        this.consecutiveRisk += 1;
        this.lastCode = code;
        const opened = this.probing || this.consecutiveRisk >= this.threshold;
        if (opened) {
...
function recordSharedRisk(responseText, scope) {
    const failure = dynamicDetailCircuitBreaker.recordFailure(responseText);
    if (!failure.isRisk) return failure;

    if (failure.opened) {
        markDiscoveryIncomplete(`${scope}触发${failure.code}采集熔断`);
        if (process.env.LOTTERY_DISCOVERY_ONLY) {
            GlobalVar.set('discoveryRiskTripped', true);
            GlobalVar.set('discoveryRiskInfo', {
                code: failure.code,
                scope,
                count: failure.count,
                openedAt: Date.now(),
        ...
        log.warn(
            '采集风控熔断',
            `${scope}连续${failure.count}次收到${failure.code}，暂停B站采集请求${Math.ceil(failure.remainingMs / 1000)}秒`
        );
    } else {
        log.warn(
            '采集风控熔断',
            `${scope}收到${failure.code} (${failure.count}/${dynamicDetailCircuitBreaker.threshold})`
        );
    }
    return failure;
}

function recordDiscoverySuccess(scope) {
    const result = dynamicDetailCircuitBreaker.recordSuccess();
    if (result.recovered) log.info('采集风控熔断', `${scope}请求已恢复`);
    return result;
}

async function requestWithDiscoveryRiskRetry(scope, requestFn, waitFn = delay) {
    if (!process.env.LOTTERY_DISCOVERY_ONLY) return requestFn();

    for (;;) {
        const gate = dynamicDetailCircuitBreaker.canRequest();
        if (!gate.allowed) return '';
        if (gate.probe) {
            log.info('采集风控熔断', `${scope}冷却结束，放行一次探测请求`);
        }

        const responseText = await requestFn();
        if (!isDiscoveryRiskResponse(responseText)) return responseText;

        const failure = recordSharedRisk(responseText, scope);
        if (failure.opened) return responseText;
        await waitFn(dynamicDetailCircuitBreaker.retryWait);
    }
}

function parseArticleSearchResponse(responseText, keyword) {
    if (isDiscoveryRiskResponse(responseText)) {
...
async function searchArticlesWithRetry(keyword, requestFn, waitFn = delay) {
    ...
    for (;;) {
        ...
        failureCount += 1;
        const failure = recordSharedRisk(responseText, `专栏搜索(${keyword})`);
        if (failure.opened) return [];
        const cooldown = process.env.LOTTERY_DISCOVERY_ONLY
            ? dynamicDetailCircuitBreaker.retryWait
            : getArticleSearch412Cooldown(failureCount);
        log.warn(
            '专栏搜索重试',
            `关键词${keyword}第${failureCount}次收到${failure.code}，${Math.ceil(cooldown / 1000)}秒后只重试该关键词`
        );
...
const bili_client = {
    ...
    async shortDynamicIdToDyid(short_id) {
        return requestWithDiscoveryRiskRetry(
            '短链接转换',
            () => get({
            ...
            if (dyid) recordDiscoverySuccess('短链接转换');
    ...
    _getOneDynamicByDyid: new Line(
        ...
        , responseText => {
            if (isDiscoveryRiskResponse(responseText)) {
                const failure = recordSharedRisk(responseText, '动态详情');
                return [
                    false,
                    DYNAMIC_DETAIL_RISK_FAILURE,
                    failure.opened
                        ? `触发${failure.code}采集熔断`
                        : `收到${failure.code} (${failure.count}/${dynamicDetailCircuitBreaker.threshold})`
                ];
            }
    ...
    async getOneDynamicByDyid(dynamic_id) {
        ...
        for (;;) {
            ...
            if (result !== DYNAMIC_DETAIL_RISK_FAILURE) break;
            if (dynamicDetailCircuitBreaker.isOpen()) return undefined;
            await delay(dynamicDetailCircuitBreaker.retryWait);
        }
        ...
        result = await this._getOneDynamicByDyid.run(dynamic_id);
        if (result === DYNAMIC_DETAIL_RISK_FAILURE) return undefined;
    ...
    recordDiscoveryRisk(responseText, scope) {
        return recordSharedRisk(responseText, scope);
    },
    recordDiscoverySuccess,
    isDiscoveryRiskResponse,
    getDiscoveryRiskCode,
    getDiscoveryRiskRetryWait() {
        return dynamicDetailCircuitBreaker.retryWait;
    },
    getDiscoveryRiskThreshold() {
        return dynamicDetailCircuitBreaker.threshold;
    },
    requestWithDiscoveryRiskRetry,
    ...
    async searchExactTopicByName(topicName) {
        const responseText = await requestWithDiscoveryRiskRetry(
            `话题搜索(${topicName})`,
            () => get({
    ...
    async getOneArticleByCv(cv) {
        const articleUrl = API.READ_CV.replace('{{cv}}', cv),
            maxAttempts = Math.max(
                ...
                process.env.LOTTERY_DISCOVERY_ONLY
                    ? dynamicDetailCircuitBreaker.threshold
                    : 0
            ),
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            if (isUsableArticleContent(content)) {
                recordDiscoverySuccess('专栏正文');
                return content;
            }

            const riskResponse = isDiscoveryRiskResponse(content);
            if (riskResponse) {
                const failure = recordSharedRisk(content, '专栏正文');
            ...
            await delay(riskResponse ? dynamicDetailCircuitBreaker.retryWait : retryWait * attempt);
        }
    ...
    async getOneOpusByDyid(dyid) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            if (isUsableArticleContent(content)) {
                recordDiscoverySuccess('合集正文');
                return content;
            }
            const riskResponse = isDiscoveryRiskResponse(content);
            if (riskResponse) {
                const failure = recordSharedRisk(content, '合集正文');
                if (failure.opened) break;
            }
            log.warn(
                '读取合集正文',
