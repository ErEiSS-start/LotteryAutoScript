...
这里的“8条”是每轮批次大小，不是每日总上限。例如固定快照中有 50 条且所有帐号都可正常参与时，每个帐号最多分 7 轮处理，前 6 轮结束后各有一次全局冷却，最后一轮处理余下 2 条。每轮日志会列出各帐号剩余数量，并根据最近3轮速度估算剩余时间。

青龙中可通过 `AUTO_IMPORT_RAY_BILIBILI_COOKIES: true` 自动读取所有启用且非空的
`Ray_BiliBiliCookies__数字` 环境变量。账号编号固定为变量数字后缀加 1；新增、更新、
禁用或删除变量会在下一次任务启动时自动生效。自动模式只使用 Ray Cookie，
`multiple_account_parm` 中的账号对象仅作为 WAIT、UA、清理和代理设置模板。
...
- 参与时先检查关注，再生成或发送评论，避免关注失败后留下无效评论。关注接口返回 `22015` 时，单个 UP 仅记为目标侧关注受限，不推送“账号异常”；同一账号在一次任务中累计达到 `follow_restriction_distinct_up_threshold` 个不同受限 UP（默认 `2`，最小 `2`）后，才发送“多UP关注受限2004”告警。

### -352 与 412 风控

不同接口的风控不再共享计数，也不会触发采集帐号切换：

- 动态详情使用普通和 desktop 两个入口。普通入口返回 `-352` 时尝试一次 desktop；两个入口都失败才算一次风控响应。
- 动态详情返回 `412` 时不请求 desktop，避免在同一出口继续追加无效请求。
- 动态详情按默认3秒、10秒、30秒、2分钟渐进冷却并重试当前动态；达到最后一档后持续使用2分钟。成功后计数清零，不跳过来源、不退出任务。
- 成功详情和同一时间正在请求的详情在当前Node.js进程内缓存，缓存键包含帐号编号和动态ID。不同帐号不会共享 `is_liked` 等账号状态，失败响应不会缓存。
- 每次详情请求使用对应动态页面作为 Referer，普通入口附带 `timezone_offset=-480`；本 Fork 暂不伪造未经验证的 WBI 或 `dm_*` 指纹。
- 专栏搜索保持独立的2分钟、3分钟、5分钟、5分钟……冷却，只重试当前关键词，成功后继续。
- 普通UID、话题、专栏正文、合集正文和短链接不参与动态详情风控；合集UID分页仅保留自己的有限 `-352` 重试。
- 动态详情的 `4101139` 和服务端 `500` 只按配置间隔重试一次；`4101152`、404、已删除及其他明确失败直接跳过。
- 话题来源使用新版Topic ID和动态分页接口；无效或无法精确解析的话题配置会被简短记录后跳过。
...
- 建议检索分页间隔保持 2 秒、动态详情间隔保持3秒；动态详情连续风控会渐进冷却并最终每2分钟探测当前动态，不再高速连续请求。
...
- PushPlus 的私信、动态 `@` 和回复中奖通知都会显示发信人昵称与 UID、中奖账号昵称
  与 UID；私信优先使用会话和账号登录信息，缺失昵称再通过 30 天本地缓存补齐。
- “预约成功并参与抽奖”、有奖调研和活动邀请不会再作为中奖通知；已有同类待领取
  误报会在下一轮检查时自动归档，明确的“恭喜中奖/请领奖”信息始终优先保留。
- 私信判断分为三档：明确中奖继续按间隔提醒；仅命中 `码、填写、提供、获得、大会员`
  等弱关键词时进入“待确认”并只推送一次；“必得红包、扫码领红包”等通用促销直接
  归档。明确的个人中奖通知优先于促销规则，因此“恭喜中奖，请扫码领取红包”仍会
  作为真实中奖提醒。
- 私信分页读取到已覆盖本轮未读数量后立即停止；遇到 `-509` 仅短暂退避并重试当前
  页，连续失败会标记本轮扫描不完整并等待下一次任务补查。
- 当脚本检测到本人在对应 B 站私信会话中发送了回复，会把记录标记为
  `acknowledged` 并停止后续提醒。
- 可部署仓库内的 `tools/qinglong-log-viewer`，登录后分别查看待领取、待确认和已归档
  私信。待确认消息只能归档或恢复，不会升级为持续提醒；操作只写入独立的
  `web_state/dismissed-wins.json`，不会标记已读、删除或修改 B 站私信；脚本在每次
  推送前都会重新读取该账本。
- 中奖管理使用独立于日志查看的页面，可显示发信人及中奖账号昵称，并直接打开
  中奖账号 B 站主页；缺失昵称通过无 Cookie 的公开名片接口补齐并缓存 30 天。
