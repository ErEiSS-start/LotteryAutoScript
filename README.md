...
- 某帐号收到 B 站评论错误 `12014`（达到当前发送上限）后，本次进程只停用该帐号的评论动作，不会再把整个帐号移出后续轮次；关注、点赞和转发仍继续执行。下次重新启动任务时会重新探测评论能力。
...
- 旧话题接口返回 404 或非 JSON 时关闭 HTTP 内部连发重试，并简短记录后跳过该话题来源，避免每个标签重复等待约一分钟和打印整页 HTML。
